  ...
  _createClass(Provider, [{
    ...
    value: function componentDidMount() {
      if (this.props.persist !== false && !this.persisted) {
        this.persisted = true;
        var storedStore = this.props.persist.storage.getItem(this.props.persist.key || 'react-context-api-store');

        if (storedStore) {
          this.setState(_extends({}, this.state, this.props.persist.statesToPersist(JSON.parse(storedStore))));
        }
      }
    }
  }]);
